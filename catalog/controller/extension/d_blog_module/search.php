<?php
class ControllerExtensionDBlogModuleSearch extends Controller
{
    private $id = 'd_blog_module';
    private $route = 'extension/d_blog_module/search';
...
//            $this->theme = $this->config->get($this->config->get('config_theme').'_directory');
    ...
    public function index() {
        ...
        $pagination->url = $this->url->link('extension/d_blog_module/search',  $url . '&page={page}', 'SSL');
        ...
        if ($page == 1) {
            $this->document->addLink($this->url->link('extension/d_blog_module/search',  $url , 'SSL'), 'canonical');
        } elseif ($page == 2) {
            $this->document->addLink($this->url->link('extension/d_blog_module/search',  $url, 'SSL'), 'prev');
        } else {
            $this->document->addLink($this->url->link('extension/d_blog_module/search',  $url . '&page='. ($page - 1), 'SSL'), 'prev');
        }

        if ($limit && ceil($post_total / $limit) > $page) {
            $this->document->addLink($this->url->link('extension/d_blog_module/search',  $url . '&page='. ($page + 1), 'SSL'), 'next');
        }
